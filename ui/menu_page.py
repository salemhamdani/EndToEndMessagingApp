...
class MenuPage:
    ...
    def show(self):
        for widget in self.context.winfo_children():
            widget.destroy()
        tk.Label(self.context, text="Menu", font=("Helvetica", 18, "bold"), bg='white').pack(pady=20)
        tk.Label(self.context, text=f"Client : {self.context.client.pseudo}", font=("Helvetica", 18, "bold"), bg='white').pack(pady=20)
    ...
    def exit(self):
        ...
        self.context.client.server_socket.close()
